...
def site():
    ...
    elif (request.vars.has_key('file') or request.vars.has_key('appurl'
          )) and not request.vars.filename:
        response.flash = \
            T('you must specify a name for the uploaded application')
    elif request.vars.filename and (request.vars.has_key('file')
                                     or request.vars.has_key('appurl')):
        mkdir = False
        try:
            appname = cleanpath(request.vars.filename).replace('.', '_')
            ...
            if mkdir:
                ...
                os.rmdir(path)
            response.flash = \
                T('unable to install application "%(appname)s"',
                  dict(appname=request.vars.filename))
    regex = re.compile('^\w+$')
    apps = [(file.upper(), file) for file in os.listdir(apath())
            if regex.match(file)]
    apps.sort()
...
def pack():
    ...
    response.headers['Content-Disposition'] = \
        'attachment; filename=web2py.app.%s.tar' % app
...
def pack_compiled():
    ...
    response.headers['Content-Type'] = 'application/x-tar'
    response.headers['Content-Disposition'] = \
        'attachment; filename=web2py.app.%s.compiled.tar' % app
    return open(filename, 'rb').read()
...
def compile_app():
    ...
    try:
        ...
        session.flash = \
            T('cannot compile. there are errors in your app. run it to debug it'
              )
    redirect(URL(r=request, f='site'))
...
def edit_language():
    ...
    for keyi in range(len(keys)):
        key = keys[keyi]
        if len(key) <= 40:
            rows.append(TR(key + ' ', INPUT(_type='text',
                        _name=str(keyi), value=strings[key], _size=40)))
        else:
            rows.append(TR(key + ':', TEXTAREA(_name=str(keyi),
                        value=strings[key], _cols=40, _rows=5)))
...
def design():
    ...
    if not response.flash and app == request.application:
        response.flash = \
            T('ATTENTION: you cannot edit the running application!')
...
def create_file():
    ...
    try:
        ...
        if path[-11:] == '/languages/':
            ...
            session.flash = \
                T('language file "%(filename)s" created/updated',
                  dict(filename=filename))
...
{{=BEAUTIFY(response._vars)}}"""\
                 % T('This is the %(filename)s template',
                     dict(filename=filename))
        elif path[-9:] == '/modules/':
            if not filename[-3:] == '.py':
                filename += '.py'
...
def errors():
    ...
    tickets = sorted(listdir(apath('%s/errors/' % app), '^\w.*'),
                     key=lambda p: os.stat(apath('%s/errors/%s' % (app,
                     p))).st_mtime, reverse=True)
...
def make_link(path):
    ...
    if os.path.isabs(tryFile) and os.path.isfile(tryFile):
        ...
        for key in editable.keys():
            if ext.lower() == editable[key] and dir.endswith(app + '/'
                     + key):
                return A('"' + tryFile + '"', _href=URL(r=request,
                         f='edit/%s/%s/%s' % (app, key, file))).xml()
    return ''
...
def ticket():
    ...
    return dict(app=app, ticket=ticket,
                traceback=TRACEBACK(e.traceback), code=e.code,
                layer=e.layer)
