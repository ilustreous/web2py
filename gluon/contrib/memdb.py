...
def assert_filter_fields(*fields):
    for field in fields:
        if isinstance(field, (SQLField, SQLXorable)) and field.type\
             in ['text', 'blob']:
...
class SQLTable(SQLStorage):
    ...
    def _create(self):
        ...
        for k in self.fields:
            ...
            if field.type[:9] == 'reference':
                ...
                ftype = \
                    self._db._translator[field.type[:9]](self._db[referenced]._tableobj)
    ...
    def update(self, id, **fields):
        for field in fields:
            if not fields.has_key(field) and self[field].default\
                 != None:
...
def obj_represent(object, fieldtype, db):
    if object != None:
        ...
        elif fieldtype == 'time' and not isinstance(object,
                datetime.time):
            time_items = [int(x) for x in str(object).strip().split(':'
                          )[:3]]
            if len(time_items) == 3:
                (h, mi, s) = time_items
            else:
...
class SQLQuery(object):
    ...
    def __init__(
        ...
        if isinstance(right, (SQLField, SQLXorable)):
            raise SyntaxError, \
                'SQLQuery: right side of filter must be a value or entity'
...
class SQLSet(object):
    ...
    def _get_table_or_raise(self):
        ...
        if len(tablenames) > 1:
            raise SyntaxError, 'SQLSet: no join in appengine'
...
def test_all():
   ...
   Insert a field
    ...
    >>> tmp = db.define_table(\"posts\",              SQLField('body','text'),              SQLField('total','integer'),              SQLField('created_at','datetime'))
