...
if os.path.exists('routes.py'):
    ...
    if 'error_message' in symbols:
        error_message = symbols['error_message']
    if 'error_message_ticket' in symbols:
        error_message_ticket = symbols['error_message_ticket']
...
def rewrite(wsgibase, URL):
    ...
    def filter_out(url):
        ...
        for (regex, value) in routes_out:
            if regex.match(items[0]):
                return '?'.join([regex.sub(value, items[0])]
                                 + items[1:])
        return url
    ...
    if 'routes_onerror' in symbols:
        wsgibase_new = lambda e, r: handle_errors(e, r,
                symbols['routes_onerror'])
    else:
