...
# Minor modification by Massimo Di Pierro so that it does not need other files
"""
WSGI HTTP Server
...
import atexit
import traceback
import socket, sys, threading, urlparse, Queue, urllib, cStringIO
import posixpath
import time
import thread
import os
...
class WSGIHandlerMixin:
    ...
    def wsgi_execute(self, environ=None):
        ...
        try:
            ...
            self.log_web2py_response(time_in)
            try:
                for chunk in result:
...
if __name__ == '__main__':
    ...
    serve(dump_environ, server_version="web2py/1.0",
