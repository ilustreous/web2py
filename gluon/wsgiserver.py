...
socket_errors_nonblocking = plat_specific_errors('EAGAIN', 'EWOULDBLOCK'
        , 'WSAEWOULDBLOCK')
...
class HTTPRequest(object):
    ...
    def _parse_request(self):
        ...
        (method, path, req_protocol) = request_line.strip().split(' ',
                2)
        ...
        if self.response_protocol == 'HTTP/1.1':
            ...
            if te:
                te = [x.strip().lower() for x in te.split(',')
                      if x.strip()]
    ...
    def communicate(self):
        ...
        try:
            while True:
                ...
                req = None
                req = self.RequestHandlerClass(self.sendall,
                    ...
                    if timeout is None or timeout < 0:
                        ...
                        if worker.isAlive():
                            ...
                            if c and not c.rfile.closed:
                                if SSL and isinstance(c.socket,
                                        SSL.ConnectionType):
...
class CherryPyWSGIServer(object):
    ...
    def __init__(
        ...
        if callable(wsgi_app):
            ...
            warnings.warn('The ability to pass multiple apps is deprecated and will be removed in 3.2. You should explicitly include a WSGIPathInfoDispatcher instead.'
                          , DeprecationWarning)
...
# #        self.socket.setsockopt(socket.SOL_SOCKET, socket.TCP_NODELAY, 1)
    ...
    def populate_ssl_environ(self):
        ...
        for (prefix, dn) in [('I', cert.get_issuer()), ('S',
                             cert.get_subject())]:
