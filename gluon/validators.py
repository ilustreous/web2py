...
class IS_GENERIC_URL(object):
    ...
    def __init__(
        ...
        if self.prepend_scheme not in self.allowed_schemes and None\
            ...
            raise Exception("prepend_scheme='"
                             + str(self.prepend_scheme)
                             + "' is not in allowed_schemes="
                             + str(self.allowed_schemes))
...
class IS_HTTP_URL(object):
    ...
    def __init__(
        ...
        for i in self.allowed_schemes:
            if i not in http_schemes:
                raise Exception("allowed_scheme value '" + str(i)
                                 + "' is not in " + str(http_schemes))
        if self.prepend_scheme not in self.allowed_schemes and None\
             in self.allowed_schemes:
            raise Exception("prepend_scheme='"
                             + str(self.prepend_scheme)
    ...
    def __call__(self, value):
        ...
        try:
            ...
            x = IS_GENERIC_URL(allowed_schemes=self.allowed_schemes,
...
class IS_URL(object):
    ...
    def __init__(
        ...
        if self.allowed_schemes:
            if prepend_scheme not in self.allowed_schemes and None\
                 in self.allowed_schemes:
                raise Exception("prepend_scheme='" + str(prepend_scheme)
                                 + "' is not in allowed_schemes="
                                 + str(self.allowed_schemes))
    ...
    def __call__(self, value):
        ...
        try:
            subMethod = eval('IS_' + str(self.mode).upper()
                              + '_URL(allowed_schemes=self.allowed_schemes, prepend_scheme=self.prepend_scheme)'
                             )
        except Exception, e:
            if re.compile("name 'IS_.*_URL' is not defined$"
                          ).match(str(e)):
                raise Exception("mode='" + str(self.mode)
                                 + "' is not valid")
            else:
                raise e
