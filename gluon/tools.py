...
class Recaptcha(DIV):
    ...
    def _validate(self):
        ...
        request = urllib2.Request(url='http://%s/verify'
                                   % self.VERIFY_SERVER, data=params,
...
"""\
        ...
        if not self.errors.captcha:
            ...
            return captcha + DIV(self.errors['captcha'], _class='error'
                                 ).xml()
...
class Auth(object):
    ...
    def __init__(self, environment, db=None):
        ...
        self.settings.verify_email_log = \
            'User %(id)s Verification email sent'
        self.settings.retrieve_username_log = \
            'User %(id)s Username retrieved'
        self.settings.retrieve_password_log = \
            'User %(id)s Password retrieved'
        ...
        if not self.settings.table_permission:
            self.settings.table_permission = \
                db.define_table(self.settings.table_permission_name,
                                db.Field('group_id'), db.Field('name',
                                default='default'),
                                db.Field('table_name'),
...
')
    ...
    def verify_email(
        ...
        if log == DEFAULT:
            log = self.settings.verify_email_log
        if next == DEFAULT:
    ...
    def change_password(
        ...
        form = form_factory(db.Field('old_password', 'password',
                            label='Old password', requires=[CRYPT(),
                            IS_IN_DB(s, '%s.password' % usern)]),
                            db.Field('new_password', 'password',
                            label='New password',
                            requires=[IS_NOT_EMPTY(), CRYPT()]),
                            db.Field('new_password2', 'password',
                            label='New password',
            ...
            def f(*a, **b):
                ...
                if not self.has_membership(group_id):
                    ...
                    if self.settings.on_failed_authorization:
                        ...
                        next = URL(r=self.environment.request,
                                   c='default', f='index')
                    redirect(next)
                return action(*a, **b)
...
class Crud(object):
    ...
    def tables(self):
        request = self.environment.request
        return TABLE(*[TR(A(name, _href=URL(r=request, args=('select',
