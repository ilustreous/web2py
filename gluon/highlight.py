    ...
    all_styles = {
        ...
        'PYTHON': (python_tokenizer, (
            ...
            ('KEYWORD',
             ...
             'color:#185369; font-weight: bold'),
            ('WEB2PY',
             re.compile(r'(request|response|session|cache|redirect|HTTP|TR|XML|URL|BEAUTIFY|A|BODY|BR|B|CENTER|CODE|DIV|EM|EMBED|FIELDSET|LEGEND|FORM|H1|H2|H3|H4|H5|H6|IFRAME|HEAD|HR|HTML|IMG|INPUT|LABEL|LI|LINK|META|OBJECT|OL|ON|OPTION|P|PRE|SCRIPT|SELECT|SPAN|STYLE|TABLE|THEAD|TBODY|TFOOT|TAG|TD|TEXTAREA|TH|TITLE|TT|T|UL|IS_ALPHANUMERIC|IS_DATETIME|IS_DATE|IS_EMAIL|IS_EXPR|IS_FLOAT_IN_RANGE|IS_INT_IN_RANGE|IS_IN_SET|IS_LIST_OF|IS_LENGTH|IS_MATCH|IS_NULL_OR|IS_NOT_EMPTY|IS_TIME|IS_URL|CLEANUP|CRYPT|IS_IN_DB|IS_NOT_IN_DB|SQLDB|SQLField|SQLFORM|SQLTABLE)(?![a-zA-Z0-9_])'
...
def highlight(
    code,
    language,
    link='/exmaples/globals/vars/',
    counter=1,
    styles={},
    attributes={},
    ):
    if not styles.has_key('CODE'):
        code_style = \
            """
        font-size: 11px;
    ...
    if not styles.has_key('LINENUMBERS'):
        linenumbers_style = \
            """
        font-size: 11px;
        font-family: Bitstream Vera Sans Mono,monospace;
